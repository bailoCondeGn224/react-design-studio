...
const Fournisseurs = () => {
  ...
  const [list, setList] = useState(initialFournisseurs);
  ...
  return (
    <AppLayout>
      ...
      <FournisseurForm open={formOpen} onOpenChange={setFormOpen} onSubmit={(data) => setList(prev => [data, ...prev])} />
          ...
          <div key={f.id} className="bg-card border border-border rounded-xl p-5 shadow-card hover:shadow-elevated transition-shadow animate-fade-in">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${f.statut === "actif" ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}>
                  {f.statut}
                </span>
