...
const Stock = () => {
  ...
  return (
    <AppLayout>
        ...
        <DialogContent className="max-w-[95vw] sm:max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          {mouvements.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground">Aucun mouvement enregistré pour cet article</p>
            </div>
          ) : (
            <div className="space-y-3">
              {mouvements.map((mouvement: any) => (
                <div key={mouvement.id} className="flex items-start gap-3 p-3 bg-secondary/30 rounded-lg hover:bg-secondary/50 transition-colors">
      ...
      {totalAlertes > 0 && (
        <div className="bg-gradient-to-r from-destructive/10 via-warning/10 to-destructive/10 border-l-4 border-destructive rounded-lg p-4 mb-6 flex items-start gap-3">
          <AlertTriangle className="w-5 h-5 text-destructive flex-shrink-0 mt-0.5" />
          <div className="flex-1">
            <h3 className="font-semibold text-foreground mb-1">Alertes Stock</h3>
      ...
      {statsRotation && (statsRotation.articlesRapides.length > 0 || statsRotation.articlesDormants.length > 0) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          {statsRotation.articlesRapides.length > 0 && (
            <div className="bg-gradient-to-br from-success/5 via-card to-card border border-success/20 rounded-xl p-5 shadow-card">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 rounded-lg bg-success/20 flex items-center justify-center">
                  <Flame className="w-4 h-4 text-success" />
                </div>
                <div>
                  <h3 className="font-semibold text-foreground">Top Produits à Forte Rotation</h3>
                  <p className="text-xs text-muted-foreground">Articles les plus vendus</p>
                </div>
              </div>

              <div className="space-y-2">
                {statsRotation.articlesRapides.slice(0, 5).map((article: any, index: number) => (
                  <div key={article.id} className="flex items-center gap-3 p-3 bg-card border border-border rounded-lg hover:shadow-md transition-shadow">
          ...
          {statsRotation.articlesDormants.length > 0 && (
            <div className="bg-gradient-to-br from-warning/5 via-card to-card border border-warning/20 rounded-xl p-5 shadow-card">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 rounded-lg bg-warning/20 flex items-center justify-center">
                  <Snowflake className="w-4 h-4 text-warning" />
                </div>
                <div>
                  <h3 className="font-semibold text-foreground">Produits Dormants</h3>
                  <p className="text-xs text-muted-foreground">Sans vente depuis +90 jours</p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                {statsRotation.articlesDormants.slice(0, 5).map((article: any) => (
                  <div key={article.id} className="flex items-center gap-3 p-3 bg-card border border-border rounded-lg hover:shadow-md transition-shadow">
                    <Snowflake className="w-5 h-5 text-warning flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-foreground truncate">{article.nom}</p>
                      <p className="text-xs text-muted-foreground">
              ...
              {statsRotation.valeurStockDormant > 0 && (
                <div className="mt-4 pt-4 border-t border-border">
                  <p className="text-xs text-muted-foreground mb-1">Valeur immobilisée</p>
                  <p className="text-lg font-bold text-warning">{formatPrix(statsRotation.valeurStockDormant)}</p>
                  <p className="text-xs text-muted-foreground">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-card border border-border rounded-lg p-4 shadow-card">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-foreground">{formatPrix(valeurTotaleStock)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-4 shadow-card">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-warning">{articlesStockFaible}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-4 shadow-card">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Rupture/Critique</p>
              <p className="text-xl font-bold text-destructive">{articlesEnRupture + articlesStockCritique}</p>
            </div>
          </div>
        </div>
      </div>
