...
const ClientForm = ({ open, onOpenChange, onSubmit, initialData = null, mode = 'create' }: ClientFormProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-md max-h-[90vh] flex flex-col p-0 bg-gradient-to-br from-background via-background to-primary/5">
          ...
          <div className="flex items-center gap-3 pr-12">
            ...
            <div>
              <DialogTitle className="text-base sm:text-lg font-bold bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text">
                <span className="md:hidden">{mode === 'edit' ? 'Modifier' : 'Client'}</span>
                <span className="hidden md:inline">{mode === 'edit' ? 'Modifier le Client' : 'Nouveau Client'}</span>
              </DialogTitle>
