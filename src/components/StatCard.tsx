...
const StatCard = ({ title, value, subtitle, icon, trend, variant = "default" }: StatCardProps) => {
  ...
  return (
    <div className={`rounded-xl p-4 sm:p-5 lg:p-6 shadow-card animate-fade-in ${variants[variant]}`}>
      ...
      <div className="space-y-0.5">
        <p className={`text-[10px] sm:text-xs font-medium uppercase tracking-wide ${isColored ? "text-primary-foreground/80" : "text-muted-foreground"}`}>
          {title}
        </p>
